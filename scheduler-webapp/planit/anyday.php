									...
									if ($res = @$connection->query($sql))
									{
										if ($res->num_rows > 0) {
										    while($row = $res->fetch_assoc()) {
										        echo "<div class=\"". $row['category']. "todo\" id=\"todo". $row['id'].  "\" onclick=\"exampl(".$row['id']. ")\"> <span style=\"text-align: center;\">".$row['time']. "</span><br/>". $row['task']. "</div>";
										    }
										}
										$res->free_result();
									}
